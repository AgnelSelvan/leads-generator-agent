...
export default function Home() {
  ...
  const loadChatHistory = async (id: string) => {
    try {
      setLoading(true);
      const res = await fetch(`http://127.0.0.1:8000/chat/${id}`);
      if (res.ok) {
        const data = await res.json();
        setMessages(data || []);
        setSessionId(id);
  ...
  const deleteKeyword = async (id: number) => {
    try {
      const res = await fetch(`http://127.0.0.1:8000/keywords/${id}`, { method: "DELETE" });
      if (res.ok) {
        fetchKeywords();
      }
    } catch (err) {
  ...
  return (
    <div className="h-screen overflow-hidden flex flex-col bg-surface-raised text-text font-sans animate-fade-in">
      ...
      <header className="w-full bg-surface border-b border-hairline-strong px-6 py-4 flex items-center justify-between sticky top-0 z-50 shadow-sm transition-colors duration-200">
        <div className="flex items-center gap-3">
          <button
            ...
            className="lg:hidden p-2 -ml-2 rounded-full text-text hover:bg-surface-strong transition-colors"
          >
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6h16M4 12h16M4 18h16"></path></svg>
          </button>
          <h1 className="text-[20px] font-[700] text-foreground tracking-tight flex items-center gap-2">
        ...
        <div className="flex items-center gap-4">
          <button
            ...
            className="p-2 rounded-full bg-surface-strong text-text hover:bg-primary hover:text-on-primary transition-all duration-200 transform hover:scale-105 active:scale-95 shadow-sm"
      ...
      <div className="flex flex-1 overflow-hidden p-4 lg:p-6 gap-6 w-full max-w-[1440px] mx-auto relative">
        ...
        {/* Sidebar Navigation */}
        <div className={`
          fixed lg:static top-0 lg:top-auto bottom-0 lg:bottom-auto left-0 z-[60] lg:z-auto transform
          ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"}
          lg:translate-x-0 transition-transform duration-300 ease-in-out
          w-64 bg-surface text-text lg:rounded-xl shadow-2xl lg:shadow-sm flex flex-col h-full flex-shrink-0 border-r lg:border border-hairline-strong
        `}>
          <div className="p-6 pb-6 border-b border-hairline flex flex-col gap-4">
            <div className="flex items-center justify-between lg:hidden">
              <span className="font-[700] text-foreground">Menu</span>
              <button
                onClick={() => setIsSidebarOpen(false)}
                className="p-2 -mr-2 rounded-full text-text hover:bg-surface-strong transition-colors"
              >
            ...
            <button
              ...
              className="w-full bg-primary text-on-primary px-4 py-3 rounded-full font-[600] text-[14px] leading-none transition-all duration-200 hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0 active:scale-95 flex items-center justify-center gap-2"
            >
          ...
          <nav className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
            ...
            <button
              ...
              className={`w-full text-left px-5 py-3 rounded-full font-[600] text-[15px] leading-none transition-all duration-200 focus:outline focus:outline-2 focus:outline-primary focus:outline-offset-2 ${activeTab === "chat"
            ...
            <button
              ...
              className={`w-full text-left flex justify-between items-center px-5 py-3 rounded-full font-[600] text-[15px] leading-none transition-all duration-200 focus:outline focus:outline-2 focus:outline-primary focus:outline-offset-2 ${activeTab === "leads"
                ? "bg-surface-strong text-foreground"
                : "text-text-mute hover:bg-surface-strong hover:text-foreground"
                }`}
            >
              ...
              {Object.keys(groupedLeads).length > 0 && (
                <span className={`text-[11px] px-2 py-0.5 rounded-full font-[700] tabular-nums ${activeTab === "leads" ? "bg-primary text-on-primary" : "bg-hairline text-foreground"
                  }`}>
            ...
            <button
              ...
              className={`w-full text-left flex justify-between items-center px-5 py-3 rounded-full font-[600] text-[15px] leading-none transition-all duration-200 focus:outline focus:outline-2 focus:outline-primary focus:outline-offset-2 ${activeTab === "keywords"
                ? "bg-surface-strong text-foreground"
                : "text-text-mute hover:bg-surface-strong hover:text-foreground"
                }`}
            >
              Keywords
              {keywords.length > 0 && (
                <span className={`text-[11px] px-2 py-0.5 rounded-full font-[700] tabular-nums ${activeTab === "keywords" ? "bg-primary text-on-primary" : "bg-hairline text-foreground"
                  }`}>
                  {keywords.length}
                </span>
              )}
            ...
            <button
              ...
              className={`w-full text-left px-5 py-3 rounded-full font-[600] text-[15px] leading-none transition-all duration-200 focus:outline focus:outline-2 focus:outline-primary focus:outline-offset-2 ${activeTab === "map"
                ? "bg-surface-strong text-foreground"
                : "text-text-mute hover:bg-surface-strong hover:text-foreground"
                }`}
            >
            ...
            <button
              ...
              className={`w-full text-left px-5 py-3 rounded-full font-[600] text-[15px] leading-none transition-all duration-200 focus:outline focus:outline-2 focus:outline-primary focus:outline-offset-2 ${activeTab === "session"
                ? "bg-surface-strong text-foreground"
                : "text-text-mute hover:bg-surface-strong hover:text-foreground"
                }`}
            >
                ...
                <div className="space-y-1">
                  {sessions.map(s => (
                    <button
                      ...
                      className={`w-full text-left px-5 py-2.5 rounded-full font-[500] text-[13px] leading-tight transition-all duration-200 truncate ${sessionId === s.session_id && activeTab === "chat"
            ...
            <div className="flex-1 flex flex-col h-full">
              ...
              <div className="flex-1 overflow-y-auto px-16 py-8 space-y-6 bg-surface">
                ...
                {messages.map((msg, i) => (
                  <div
                    key={i}
                    className={`flex flex-col max-w-[70%] animate-message-pop ${msg.role === "user"
                  ...
                  >
                    ...
                    <div
                      className={`p-5 text-[16px] font-[400] leading-relaxed rounded-2xl ${msg.role === "user"
